...
#
# It takes quite a while to run this script, it recomended to only run a single scenario at a time. 

# 0. Set Up ------------------------------------------------------------------------------------------------
# DATA_DIR <- here::here('output', 'rcmipII', 'raw-ensemble')
# DATA_DIR <- '/Volumes/KDBackup/rcmipII' # Use the data stored on the external hard drive 
...
lapply(data_files, function(f, vars = c("Tgav", "Ftot", "FCO2", "heatflux")){
